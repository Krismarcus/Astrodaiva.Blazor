...
No Text is a per-date visibility switch (HideEventText), separate from aspect visibility. Checking it removes the entire authored event frame from both public and draft calendar views while preserving all LT/EN titles and descriptions. Unchecking it restores the saved content. The setting survives imports, backups, and publication. Empty dates continue to render no frame. Text inputs update the draft while typing so the final edit is retained when toggling visibility or opening preview.

Event Text and its LT/EN descriptions display authored content only; an empty title is never filled with automatic planet/aspect prose. Show aspect symbols is an independent, off-by-default display preference saved with the snapshot. When enabled, a separate row beneath the planet positions shows each planet pair with its conjunction, opposition, trine, square, or sextile symbol. It contains no visible labels, event titles, or times; accessible labels describe the symbols for screen readers. Imported days use their edited exact-event list, while older dates use their saved PlanetEvents. Unsupported aspects are omitted. Aspects are excluded from the detailed Exact events list so their visibility follows this one switch. Preview draft reflects the preference, and Publish changes applies it publicly. Imports and resets preserve authored text and the preference.
